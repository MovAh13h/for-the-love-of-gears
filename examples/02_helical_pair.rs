...
fn main() {
    ...
    hrow(
        "Reference diameter",
        pinion.reference_diameter(),
        wheel.reference_diameter(),
        "mm    mt·z",
        w,
    );
    hrow(
    ...
    println!(
        ...
        pinion.center_distance_to(&wheel)
    );
    println!(
        "  Gear ratio            {:>8.3}:1",
    ...
    println!(
        ...
        pinion.tooth_thickness()
    );
    println!(
        "  Thinned thickness     {:>8.4} mm  s' = s − (jt/2)·cos ψ",
        pinion.thinned_tooth_thickness(jt).unwrap()
    );
    println!(
        "  Normal backlash jn    {:>8.4} mm  jt·cos αt·cos ψ",
        pinion.normal_backlash(jt).unwrap()
    );
}

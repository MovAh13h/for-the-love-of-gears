...
#[test]
fn circular_and_diametral_are_consistent() {
    let m = 3.0;
    let product = CircularPitch::from_module(m).value() * DiametralPitch::from_module(m).value();
    assert!((product - PI * 25.4).abs() < 1e-10);
}
